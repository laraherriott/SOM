...
loss_list, loss_validate, top_pred = train(model, train_loader, validate_loader, epochs, device, optimiser, loss_function, max_length)
if len(loss_validate) == 0:
    loss_validate = [0]*len(loss_list)

# save trained model
model_output_dir = '../models'
model_file_name = 'atom_type_test'
torch.save(model.state_dict(), os.path.join(model_output_dir, model_file_name))

# make predictions for test dataset
accuracy = predict(model, device, test_loader)

df_training_history = pd.DataFrame({"loss": loss_list, "validation_loss": loss_validate,  "accuracy": top_pred})
df_training_history.to_csv(os.path.join(os.path.join("../output","training_logs"), model_file_name + ".csv"))

test_results = pd.DataFrame({"accuracy": accuracy})
